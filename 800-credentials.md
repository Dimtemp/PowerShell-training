...
1. Inspect the credential variable: ```$cred```. Notice the password is a secure string (System.Security.SecureString).
...
## Task 2: retrieve the credential object
1. Run this command to retrieve the credentials: ```$credFromDisk = Import-CliXml cred.xml```
1. Inspect the credential variable: ```$credFromDisk```. Notice the password is (still) a secure string (System.Security.SecureString).

## Task 3: store a password securely:
1. Run this command to enter a password securely: ```$SecurePassword = Read-Host -AsSecureString -Prompt 'Please enter password'```
1. Run this command to store the password: $SecurePassword | Export-Clixml -Path password.xml.
1. Inspect the contents of the XML file: ```Get-Content password.xml```
1. Notice the password is stored in encrypted form.

## Task 4: retrieve the password
1. Run this command to retrieve the secure password: ```$passwordFromDisk = Import-CliXml password.xml```
1. Inspect the password variable: ```$passwordFromDisk```. Notice the password is (still) a secure string (System.Security.SecureString).
